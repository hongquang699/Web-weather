...
Features:
  ...
  - Target Repository: https://github.com/hongquang699/Web-weather
"""
import os
import sys
import subprocess
...
DEFAULT_REPO_URL = "https://github.com/hongquang699/Web-weather.git"
...
def _load_github_token():
    """Read GitHub Token securely from environment variables or .env file (hidden from git)"""
...
def git_push(commit_message: str = None) -> dict:
    ...
    try:
        ...
        ignore_dirs = {'build', '.gradle', '.idea', 'dist', 'bin', '.git', '__pycache__', '.kotlin', 'venv'}
        ...
        # 4. git commit
        result = subprocess.run(
            ["git", "-c", "safe.directory=*", "commit", "-m", commit_message],
            cwd=PROJECT_ROOT,
            capture_output=True, text=True, encoding="utf-8", timeout=30
        )
